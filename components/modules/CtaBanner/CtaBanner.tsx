...
export default async function CtaBanner({ data, title, moduleTitle }: { data?: CtaBannerContent; title?: ReactNode; moduleTitle?: string } = {}) {
      ...
      <div className="container-1200 relative gw-section">
        ...
        {moduleTitle && <ModuleTitle id="gw-cta-banner-module-title">{moduleTitle}</ModuleTitle>}
          <ScrollFadeIn delay={0.3}>
            <div
              id={`${banner.id}-ctas`}
              className={`flex flex-col items-center justify-center gap-3 sm:flex-row ${title || moduleTitle ? "mt-0" : "mt-10"}`}
            >
              {banner.ctas.map((cta) => {
